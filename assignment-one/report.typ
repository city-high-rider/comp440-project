...
In contrast, I was not able to complete this lemma in Coq. The difficulty is not that the statement is false or that Coq is incapable of expressing it, but rather that this kind of argument does not align well with the way the problem was modelled. Recall that we chose to treat the dependency graph as an abstract relation together with properties such as acyclicity, not as an inductively defined data structure that can be traversed like we did in Idris. The model we chose for Coq was not arbitrary, and it significantly simplified other parts of the development. In particular, we did not need to parameterise scheduler states over a specific graph, and state transitions did not need to carry around or reconstruct as many proofs. This made many of the important objects, such as states, steps, and traces nicer to work with. However, this convenience comes at a cost: the graph is not available as an inductive structure, so arguments that rely on traversing it (such as these two lemmas) become substantially more difficult to formalize. Doing so would require building a non-trivial chain of dependencies in the transitive closure of the relation and showing that it eventually loops, which is much more involved than the direct recursive search used in the Idris proof. 

Now we have seen two proofs, one more suited for tactics, and another more suited for computation. The relational and property based style of reasoning seen in Coq was much more natural for high-level pen-and-paper arguments, but made computational proofs like this one much harder to express. In contrast, Idris allows the proof to be written directly as a terminating program, making this kind of constructive search argument much more straightforward to encode. However, it can make proofs that involve lots of symbolic manipulation more tedious to write, harder to read, and more cluttered than a tactic based approach.
