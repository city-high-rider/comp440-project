...
Both Idris and Coq provide several interactive features that assist with theorem proving. Idris offers lots of independent functionalities which help directly construct terms, such as generating function bodies from type signatures, case-splitting on variables, inspecting the types of holes and local variables, lifting holes into separate lemmas, performing expression search, and cheking the entire file for correctness. These tools support Idris' constructive workflow well: after defining the type of a function, it is possible to generate the body, split on arguments to create sub-goals, and inspect the placeholder holes to determine the context in seconds with few key strokes. In some cases, the type checker is even able to automatically discharge certain input combinations for being impossible. Using holes allows the programmer to get feedback from the type checker, even if the implementation is not in a complete state. However, in practice, Idris' interactive edititng features do have a few notable sharp edges. Raising a hole to a separate lemma often introduces an excessive number of parameters, since everything in scope is included by default. Ocasionally, case-splitting can produce branches that the totality checker does not recognise as covering, which requires some manual restructuring. Furthermore, when working with complicated expressions (especially arithmetic ones), the type information displayed for holes can become verbose and hard to understand.

There are also some quirks in the behavior of the unifier that affect usability. For example, record projections are not always expanded as expected, and let-bound expressions are not always treated as definitionally equal to their unfolded forms. In some cases, expressions that appear identical fail to unify due to mismatching implicit arguments, while in others, terms are expanded too eagerly, making them cumbersome to manipulate.

Coq provides a cohesive interactive environment centered around tactics. When in proof mode, a panel will display the current proof state, including the goal and current hypotheses. The user can incrementally apply tactics and observe how the proof state changes. Importantly, a completed proof can be stepped through from start to finish without having to modify any code. This is unlike Idris, where exploring the intermediate states of a proof often requires manually inserting holes, as seen in the proof of lemma 4. Coq also features tactics like `firstorder` and `lia` which attempt to solve proofs automatically. This is similar to Idris' expression search, however they are invoked as tactics rather than as an operation from the language server.
